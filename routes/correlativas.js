...
function pickCareerPlan(req){
  const qCareer   = String(req.query.career || '').trim();
  const qPlanRaw  = req.query.plan;
  const userCareer = String((req.user && req.user.career) || '').trim();
  const userPlanRaw = (req.user && req.user.plan);

  const qPlan = parseInt(String(qPlanRaw ?? '').trim(), 10);
  const userPlan = parseInt(String(userPlanRaw ?? '').trim(), 10);
...
// =========================
// ✅ API: correlativas DONE (persistente en Turso)
// GET  /app/correlativas/api/done
// POST /app/correlativas/api/done  { subjectId, done }
// =========================
router.get('/correlativas/api/done', async (req, res) => {
  try{
    const u = (req.user || (req.session && req.session.user) || null);
    if (!u || u.role === 'guest' || !u.id) return res.status(401).json({ ok:false, ids:[] });
    ...
    const ids = (rows || []).map(r => String(r.subject_id));
    return res.json({ ok:true, ids });
  }catch(e){
    console.error('GET /correlativas/api/done error:', e);
    return res.status(500).json({ ok:false, ids:[] });
  }
});

router.post('/correlativas/api/done', express.json(), async (req, res) => {
  try{
    const u = (req.user || (req.session && req.session.user) || null);
    if (!u || u.role === 'guest' || !u.id) return res.status(401).json({ ok:false });
    ...
    console.error('POST /correlativas/api/done error:', e);
