...
(function(){
  ...
  /* ============ Pasos (tus textos originales) ============ */
  const steps = [
    { key:'ruleta',   title:'Juegos para practicar', pos:'right',
      body:'Cada una de estas <strong>cards</strong> es un juego donde practicás con <strong>preguntas de la materia que quieras</strong>. Es ideal para cuando estás <strong>harto de estudiar</strong> pero no te querés sentir <strong>culpable por cortar el estudio</strong>, porque seguís repasando de una forma más entretenida.' },
  ...
  /* ============ Localizar rectángulos según tu layout de Juegos ============ */
    /* ============ Localizar rectángulos según tu layout de Juegos ============ */
  function getRouletteCardRect(){
    const el = document.querySelector('.game-card[data-game="roulette"]')
           || document.querySelector('#gamesHub .game-card[aria-hidden="false"][data-game="roulette"]')
           || document.querySelector('.games-stage .game-card[data-game="roulette"]');
    if (!el) return null;
    return rectOf(el);
  }

  function computeStepRect(step){
    if (step.key === 'ruleta' || step.key === 'puntos' || step.key === 'materias'){
      const rr = getRouletteCardRect(); if (rr) return rr;
    }
    // Fallback centrado
