// public/js/materias-tour.js
// v7.0 – NO eleva UI por encima del overlay + bloqueo total de interacciones durante el tour
(function(){
  // ---------- Helpers base ----------
  ...
  function ensureCSS(){
    ...
    const css = `
      ...
      /* TOOLTIP SIEMPRE VISIBLE/ENCIMA */
      .tour-tooltip{
        position:fixed;
        max-width:420px;
        padding:16px 18px;
      ...
      .tour-close{
        position:fixed; top:16px; right:16px; z-index:2147483667;
        width:40px; height:40px; border-radius:50%; background:#fff; color:#111827; border:none; cursor:pointer;
        box-shadow:0 10px 24px rgba(0, 0, 0, 0.12); font-weight:800; display:none; line-height:40px;
        pointer-events:auto;
      }

      /* Spotlight que oscurece todo alrededor (donut) */
      #tourRectProxy{
        position:fixed; z-index:2147483655; pointer-events:none;
        border-radius:12px;
        outline:4px solid #ffffff;
        box-shadow:
          0 0 0 8px rgba(255,255,255,.90),
          0 0 40px 10px rgba(255,255,255,.65),
          0 0 0 9999px rgba(15,23,42,0.23);
      }

      /* ===== Bloqueo total de interacciones durante el tour ===== */
      body.tour-active *{
        pointer-events:none !important;
      }
      body.tour-active #tourBackdrop,
      body.tour-active #tourRectProxy,
      body.tour-active #tourTooltip,
      body.tour-active #tourClose{
  ...
  function ensureOverlay(){
    ...
    if (!tooltip){
      ...
      document.body.appendChild(tooltip); // después del proxy
    }
    if (!closeBtn){
  ...
  function computeStepRect(step){
    ...
    if (step.key === 'logout'){
      // buscar botón real y solo caer a anclaje si no aparece
  ...
  const steps = [
    ...
    { key:'usuario',        title:'Cuty (mascota)',         targets:S.usuario,    pos:'right',  body:'Ella será tu pequeña amiga; se quedará cerquita de ti para darte paz y calidez mientras te concentras en tus cosas. ✨🐾.' },
  ...
  function placeAndShow(els, step){
    ...
    // Colocar SIEMPRE fuera y por encima
    requestAnimationFrame(()=> {
      placeTooltipOutside(els.tooltip, rect, step.pos);
      els.tooltip.style.zIndex = 2147483666; // refuerzo
    });
  }

  function startTour(){
    ensureCSS();
    // Marca global para bloquear interacciones fuera del tutorial
