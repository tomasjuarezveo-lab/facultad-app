// public/js/tutorial-profesores.js
...
(function(){
  ...
  function onProfesores(){
    return location.pathname.indexOf("/app/profesores") === 0;
  }
  ...
  /* ============ Pasos (adaptados a tu HTML actual de Profesores) ============ */
  const steps = [
    {
      key:"busqueda",
      title:"Buscá un profesor",
      pos:"bottom",
      body:"Usá el buscador para encontrar un profesor por <strong>nombre</strong> rápidamente.",
      target:"#qInput, .topbar .search, form.search, input[type='search']"
    },
    {
      key:"top",
      title:"Top del mes",
      pos:"right",
      body:"Acá aparecen los profesores más destacados de los <strong>últimos 30 días</strong>.",
      target:"#topMonth, .top-month, .tpStage, .tpHead"
    },
    {
      key:"grid",
      title:"Los profesores",
      pos:"bottom",
      body:"Tocá cualquiera para abrir su ficha, ver reseñas y también <strong>calificar</strong>.",
      target:".listScroll, #listScroll, .prof-list, .profes-list, .prof[data-prof-id]"
    }
  ];
  ...
  function computeStepRect(step){
    ...
    if (el){
      // Si el elemento está fuera de la vista, lo acercamos para que el spotlight tenga sentido
      try{
        const r = el.getBoundingClientRect();
