// public/js/profesores-tour.js
// Tutorial para Profesores — spotlight idéntico a Juegos/Autoevaluaciones (fondo oscuro con “agujero”)
...
(function(){
  ...
  function onProfesores(){ return location.pathname.indexOf("/app/profesores") === 0; }

  /* ============ CSS igual al de juegos/autoevaluaciones (clave: proxy con sombra 9999px) ============ */
  function ensureCSS(){
    ...
    const css = `
      ...
      /* Spotlight que oscurece TODO menos el rectángulo resaltado (mismo efecto que juegos/autoeval) */
      #tourRectProxy{
  ...
  /* ============ Pasos (adaptados a Profesores) ============ */
  const steps = [
    { key:"estrellas", title:"Filtrar por estrellas", pos:"bottom",
      body:"Utilizá los botones para encontrar a los profesores según su <strong>calificación</strong>.",
      target:"#segStars, .seg-stars, [data-filter='stars']"
    },
    { key:"top", title:"Los mejores del mes", pos:"right",
      body:"Aquí verás a los 5 profesores mejor calificados de este mes, los más <strong>destacados</strong>.",
      target:"#topCard, .top-card, [data-top='month']"
    },
    { key:"grid", title:"Los profesores", pos:"bottom",
      body:"Tocá cualquiera para abrir su ficha y poder conocer mas sobre el profesor e incluso <strong>calificarlo</strong>.",
      target:"#profGrid, .prof-grid, [data-grid='profes']"
    }
  ];
  ...
  function placeAndShow(els, step){
    ...
    // Spotlight igual a juegos/autoeval (borde blanco + glow + oscurecimiento alrededor)
    ...
    requestAnimationFrame(()=>{
      // refuerza no-dim (como juegos/autoeval)
      try{ if (typeof forceUndim === "function") forceUndim(); }catch(e){}
  ...
  function boot(){
    if (!onProfesores()) return;
    window.addEventListener("open-tutorial:profesores", startTour);
    ...
    // Debug opcional:
    window.__startProfesoresTour = startTour;
  }
