// scripts/overwrite_conta7_edges_partial.js
// Inserta correlativas de Contabilidad Plan 7 omitiendo con aviso lo que no matchea.
// Uso: node scripts/overwrite_conta7_edges_partial.js

const { init, all, run } = require('../models/db');

// Normalización y similitud (igual que antes)
...
// === ALIAS MINIMOS SEGUROS (podés ampliar con lo que decidas) ===
const ALIAS = new Map([
  ...
  // Ejemplos sugeridos (descomenta si te sirven):
  // ["Administración Pública", "Contabilidad del Sector Público"],
  // ["Derecho Empresario", "Derecho Privado"],
  // ["Diseño de Sistemas de Información", "Sistema de Información Contable de Apoyo a las Operaciones"],
  // ["Marketing Estratégico", "Comercialización"],
  // ["Marketing Táctico y Operativo", "Comercialización"],
  // ["Tecnología Informática y Sistemas de Información para la Dirección", "Sistema de Información Contable para la Toma de Decisiones"],
]);

// === MAP con tu pedido (igual al anterior) ===
...
const MAP = {
  ...
  [ADMIN_III]: { cursada: [], final: [ADMIN_I, ADMIN_II, DER_CONST, INTERP_EE, "Introducción a las Ciencias Sociales y al Conocimiento Científico", MAT_I, MICRO_I] },
  [ADMIN_PUB]: { cursada: [], final: [ADMIN_I, COMPORT, DER_CONST, "Introducción a las Ciencias Sociales y al Conocimiento Científico", MAT_I, MICRO_I] },
  [EST_APL]: { cursada: [], final: [ADMIN_I, DER_CONST, INTRO_ECO, "Introducción a las Ciencias Sociales y al Conocimiento Científico", MAT_I, MAT_II, MICRO_I] },
  [DER_EMP]: { cursada: [], final: [ADMIN_I, DER_CONST, DER_PRIV, INTRO_ECO, "Introducción a las Ciencias Sociales y al Conocimiento Científico", MAT_I, MICRO_I] },
  [MAT_DEC]: { cursada: [], final: [ADMIN_I, DER_CONST, INTRO_ECO, "Introducción a las Ciencias Sociales y al Conocimiento Científico", MAT_I, MAT_II, MICRO_I] },
  ...
  [TRAB_SOC]: { cursada: [], final: [ADMIN_II, COMPORT, DER_CONST, DER_PRIV, FIN_PUB, HIST_ECO1, INTERP_EE, "Introducción a las Ciencias Sociales y al Conocimiento Científico", MACRO_I, MAT_I, MAT_II, MICRO_I] },
  [FIN_EMP]: { cursada: [], final: [ADMIN_III, COMPORT, DER_PRIV, FIN_PUB, HIST_ECO1, MACRO_I, MAT_DEC] },
  [PSICO_ORG]: { cursada: [], final: [ADMIN_II, COMPORT, DER_CONST, DER_PRIV, FIN_PUB, HIST_ECO1, INTERP_EE, "Introducción a las Ciencias Sociales y al Conocimiento Científico", MACRO_I, MAT_I, MAT_II, MICRO_I] },
...
(async () => {
  try{
    ...
    function match(name){
      const n=normalize(alias(name)), np=normalize(withoutParens(alias(name)));
      return byNorm.get(n) || byNormNoPar.get(np) || null;
    }

    // Borramos TODO el scope primero (para que quede limpio)
    await run(`DELETE FROM correlatives_edges
               WHERE subject_id IN (SELECT id FROM subjects WHERE career=? AND plan=?)`,
               [CAREER, PLAN]);

    let ins=0, skippedSubjects=[], skippedDeps=[];
    for (const [subjName, reqs] of Object.entries(MAP)){
      const subj = match(subjName);
      if (!subj){ skippedSubjects.push(subjName); continue; }
      for (const depName of (reqs.cursada||[])){
        const dep = match(depName);
        if (!dep){ skippedDeps.push(`${subjName} ←(cursada)— ${depName}`); continue; }
        await run(`INSERT INTO correlatives_edges (subject_id, depends_on_id, req_type) VALUES (?,?,?)`,
                  [subj.id, dep.id, 'cursada']); ins++;
      }
      for (const depName of (reqs.final||[])){
        const dep = match(depName);
        if (!dep){ skippedDeps.push(`${subjName} ←(final)— ${depName}`); continue; }
        await run(`INSERT INTO correlatives_edges (subject_id, depends_on_id, req_type) VALUES (?,?,?)`,
                  [subj.id, dep.id, 'final']); ins++;
      }
    }
    ...
    if (skippedSubjects.length){
      console.log('\n⚠ Materias destino omitidas por no existir en Contabilidad/7:');
      for (const s of skippedSubjects) console.log('  -', s);
    }
    if (skippedDeps.length){
      console.log('\n⚠ Prerrequisitos omitidos por no existir en Contabilidad/7:');
      for (const s of skippedDeps) console.log('  -', s);
    }
    process.exit(0);
  }catch(e){
    console.error('❌ Error overwrite_conta7_edges_partial:', e);
    process.exit(1);
  }
})();